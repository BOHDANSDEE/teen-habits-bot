...
const PROBLEM_OPENERS = Object.freeze([
  ...
  "буває, що ти ",
  "у деякі дні ти ",
  "у звичних ситуаціях ти "
]);

const GAIN_FRAMES = Object.freeze([
  "{context} {benefit}",
  "{context} коротка вигода проста: {benefit}",
  "{context} цей спосіб дає коротку вигоду: {benefit}",
  "{context} тут є проста вигода: {benefit}",
  "{context} старий спосіб тримається на вигоді: {benefit}",
  "{context} причина не змінюватися одразу проста: {benefit}",
  "{context} звичний спосіб здається зручним, бо {benefit}",
  "{context} залишатися по-старому легше, бо {benefit}",
  "{context} коротке полегшення таке: {benefit}",
  "{context} старий вибір приваблює тим, що {benefit}"
]);

const MEANING_FRAMES = Object.freeze([
  "{context} через це {cost}",
  "{context} наслідок помітний: {cost}",
  "{context} це відбивається прямо: {cost}",
  "{context} проблема впливає так: {cost}",
  "{context} у підсумку {cost}",
  "{context} головний наслідок такий: {cost}",
  "{context} це видно по тому, що {cost}",
  "{context} у житті це означає: {cost}",
  "{context} ця звичка веде до того, що {cost}",
  "{context} результат старого способу такий: {cost}"
]);
...
const TOPIC_OVERRIDES = Object.freeze({
  "social-media/scroll": Object.freeze({
    ...
    permission: "самому визначати, скільки часу провести у стрічці",
  ...
  "social-media/auto": Object.freeze({
    ...
    alternative: "перед відкриттям стрічки запитати себе, навіщо заходиш",
  ...
  "rest/scroll": Object.freeze({
    ...
    alternative: "відкласти телефон і дати собі коротку паузу без стрічки",
...
function benefitClause(benefit) {
  ...
  if (/^відчуття\s+/u.test(value)) return `ти отримуєш ${value}`;
  if (/^(готову|готовий|просту|простий|приємний|приємне|швидкий|швидке|новизну)\s+/u.test(value)) {
...
function buildProblem(theme, topic, style) {
  ...
  return style < 10
    ? `${first} ${second}`
    : `${first} ${second} ${sentence(`Так тобі складніше ${value.permission}`)}`;
}

function buildGain(theme, topic, style) {
  const value = parts(theme, topic);
  const local = style % 10;
  const first = sentence(fill(GAIN_FRAMES[local], {
    context: contextFor(theme),
    benefit: benefitClause(value.benefit)
  }));
  const conclusion = style < 10
    ? "Тому тобі вигідно залишатися у такому способі дій."
    : "Тому тобі вигідно залишатися у такому способі дій — коротка користь з’являється одразу.";
  return `${first} ${conclusion}`;
}
