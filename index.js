...
Обери рівень. У пробній версії поки додано один базовий рівень для кожної теми.`,
...
2. Обираєш рівень.
3. Отримуєш одну з варіацій: стан, проблема, можлива вторинна вигода, значення в житті, практичний крок та афірмація.
4. Кнопка «Хочу ще рішення» переводить у випадково обрану суміжну тему, щоб продовжити розбір без нового старту.

Фрази рандомізуються, тому повторний запуск може дати іншу версію тієї самої теми.`,
      { reply_markup: mainMenuKeyboard() }
    );
    return;
  }
...
bot.on("message", async (msg) => {
  ...
  await safeSend(
    msg.chat.id,
    "Зараз пробна версія працює через кнопки. Натисни /start, щоб відкрити меню.",
    { reply_markup: mainMenuKeyboard() }
  );
});
