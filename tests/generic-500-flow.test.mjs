...
const PHYSICAL = /скрон|потилиц|лоб|оч|повік|щелеп|ши|плеч|груд|ключиц|дих|спин|лопат|поперек|передпліч|зап’яст|зап'яст|рук|кист|долон|пальц|жив|таз|стегн|литк|щиколот|п’ят|п'ят|ног|стоп|колін|тіл|м’яз|м'яз/iu;
const BODY_ZONES = [
  /скрон/iu,
  /потилиц/iu,
  /лоб/iu,
